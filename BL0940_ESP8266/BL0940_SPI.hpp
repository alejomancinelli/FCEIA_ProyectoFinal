...
{ 
  private:
    ...
    uint8_t _calcCheckSum(uint8_t opCode, uint8_t address, uint8_t *data, size_t dataLen);
    
    void _writeCommand(uint8_t *data, size_t len);
    void _readCommand(uint8_t *tx, size_t txLen, uint8_t *rx, size_t rxLen);
    ...
    const float R8 = 20000.0;  // [Ohm]
    const float R9 = 20000.0;  // [Ohm]
    const float R10 = 20000.0;  // [Ohm]
    const float R11 = 20000.0;  // [Ohm]
    const float R12 = 20000.0;  // [Ohm]
