...
def get_voltage(raw_data):
    ...
    R7 = 24.9  # Ω
    voltage = (raw_data * Vref * R8_R12_total) / (79931.0 * R7)
    return voltage

def get_current(raw_data):
    Vref = 1.218
    R5 = 3.3
    Rt = 2000.0
    denominator = (324004.0 * R5 * 1000.0) / Rt
    current = (raw_data * Vref) / denominator
    return current
...
try:
    ...
    if ser.is_open:
        ...
        while True:
            # Voltage
            raw_vrms = read_register(ser, 0x06)  # V_RMS
            ...
            # Current
            raw_irms = read_register(ser, 0x04)  # I_RMS
            if raw_irms is not None:
                current = get_current(raw_irms)
